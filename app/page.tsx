...
function buildHTML(tabs: Tab[]) {
  const btns = tabs.map(
    (t, i) =>
      `<button role="tab" id="tab-${i}" aria-controls="panel-${i}" aria-selected="${
        i === 0 ? "true" : "false"
      }" onclick="selectTab(${i})">${t.title}</button>`
  ).join("\n      ");
...
export default function Home() {
  ...
  const copy = async () => {
    ...
    alert("Copied! Paste into Hello.html");
  };
  ...
  return (
    <section className="grid-2">
      ...
      <div className="left">
        ...
        <div className="kv">
          <div>
            ...
            <button className="addbtn" onClick={addTab}>[+]</button>
            ...
            <div className="box" style={{ maxWidth: 360 }}>
              <textarea
                ...
                onChange={(e) => {
                  ...
                  setTabs((ts) =>
                    ts.map((t, idx) => (idx === current ? { ...t, content: v } : t))
                  );
                }}
                rows={10}
                style={{
      ...
      <div className="right">
        ...
        <pre id="out" className="codearea" aria-label="Generated code">
          {html}
        </pre>
      </div>
    </section>
